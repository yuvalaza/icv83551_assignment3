...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    for img in images:
        ...
        pred_classes.append(class_texts[sims.argmax().item()])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        imgs_processed = torch.stack([clip_preprocess(Image.fromarray(img)) for img in images]).to(device)
        self.img_feats = clip_model.encode_image(imgs_processed)
        self.img_feats = self.img_feats / self.img_feats.norm(dim=1, keepdim=True)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        sims = (txt_feats @ self.img_feats.T).squeeze(0)
...
class DINOSegmentation:
    ...
    def inference(self, X_test):
        ...
        ############################################################################
        self.net.eval()
        pred_classes = self.net(X_test).argmax(dim=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes